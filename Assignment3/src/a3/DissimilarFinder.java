...
/**
 * Represents the Dissimilar Finder that is a subclass of Finder
 */
public class DissimilarFinder extends Finder{
   ...
   */
  public DissimilarFinder() {
    super();
  }
  
  /**
   * The overridden method from the Finder Interface which finds the
   * most dissimilar pair of users and the score
   * 
  ...
  public void findScore(Matrix userUserMatrix) {
    try {
      // set the minimum value of dissimilarity scores
      score = 1;
        ...
        for(int user2 = user1; user2 < numberOfUser; user2++) {
          ...
          if(user1 != user2) {
            ...
            if(score > userUserScore) {
              /* if the userUser score is less than the current score
               * then we'll remove all pairs in userPairs array
               * and add the current userPair
               */ 
              userPairs.clear();
              userPairs.add(userPair);
              // replace the score with the current lowest dissimilarity score
              score = userUserScore;
              
            } else if(score == userUserScore) {
              /* check if the current userUser score
               * is equal to the score then add the
               * current user pair to the userPair array
               */
               userPairs.add(userPair);
            }
          }
